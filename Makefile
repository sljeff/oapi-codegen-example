...
build_debug: $(DIR_BUILD)
	CGO_ENABLED=1 $(GO) build $(DEBUG_FLAGS) -o $(FILE_DEBUG_BIN) $(BIN_SRC)

run: build_debug
	./$(FILE_DEBUG_BIN) $(COMMAND) $(ARGS)

build_release: $(DIR_DIST)
	CGO_ENABLED=1 $(GO) build $(RELEASE_FLAGS) -o $(FILE_RELEASE_BIN) $(BIN_SRC)
...
genswagger:
	mkdir -p $(OAPI_TARGET)
	oapi-codegen -package oapi -generate=types,client,server,spec,skip-prune -o $(OAPI_TARGET)$(OAPI_TARGET_FILENAME) $(OAPI_SPEC)
